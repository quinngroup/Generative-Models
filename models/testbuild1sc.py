...
def eigenDecomposition(A, plot = True):
    ...
    eigenvalues, eigenvectors = eigsh(L, k=2000, which="SM", sigma=1.0, maxiter=5000)
...
def test(epoch, max, startTime):
    ...
    if(epoch == max):
        ...
        if (args.spectral == True) :
            coord=torch.Tensor.cpu(zTensor).numpy()
            k,_,_=eigenDecomposition(getAffinityMatrix(coord))
            print(f'Optimal number of clusters {k}')
            spectral = SpectralClustering().fit(coord)
            print(spectral)
