...
class VAE(nn.Module):
    ...
    def q_z(self, x):
        ...
        x = F.max_pool2d(self.p1(self.conv1(x)), (2,2))
        
        #(8,13,13) -> (16,12,12) -> (16,6,6)
        x = F.max_pool2d(self.p2(self.conv2(x)), (2,2))
        
        #(16,6,6) -> (32,4,4)
        x = self.p3(self.conv3(x))
        
        #(32,4,4) -> (64,2,2)
        x = self.p4(self.conv4(x))
    ...
    def p_x(self, z):
        ...
        x = PReLU(self.fc1(z))
        x = x.view(-1,1,2,2)
        x = PReLU(self.convt1(x))
        x = PReLU(self.convt2(x))
        x = PReLU(self.convt3(x))
        x = PReLU(self.convt4(x))
        '''
        return x
    ...
    def log_p_z(self,z):
        ...
        print
        # calculate params
        X = self.p5(self.means(self.idle_input))
