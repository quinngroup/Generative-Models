...
"""
VampPrior implementation with Spatial Broadcast Decoder for use with MNIST dataset, with Pixelwise convolutions added 

@author Davis Jackson, Meekail Zain
"""
...
class VAE(nn.Module):
    def __init__(self, input_length, lsdim, pseudos, beta, gamma, batch_size, device):
        ...
        #Pixelwise Convolutions
        self.pxl1 = nn.Conv2d(8,8,1)
        self.pxl2 = nn.Conv2d(8,8,1)    
        
        #(8,13,13) -> (16,12,12)
        self.conv2 = nn.Conv2d(8, 16, 2)
        
        #Pixelwise Convolutions
        self.pxl3 = nn.Conv2d(16,16,1)
        self.pxl4 = nn.Conv2d(16,16,1)
        
        #(16,6,6) -> (32,4,4)
        self.conv3 = nn.Conv2d(16, 32, 3)
        
        #Pixelwise Convolutions
        self.pxl5 = nn.Conv2d(32,32,1)
        self.pxl6 = nn.Conv2d(32,32,1)
        
        #(32,4,4) -> (64,2,2)
        self.conv4 = nn.Conv2d(32, 64, 3)
        
        #Pixelwise Convolution
        self.pxl7 = nn.Conv2d(64,64,1)
        self.pxl8 = nn.Conv2d(64,64,1)
    ...
    def q_z(self, x):
        ...
        #Pixelwise
        x = F.leaky_relu(self.pxl1(x))
        x = F.leaky_relu(self.pxl2(x))
        
        #(8,13,13) -> (16,12,12) -> (16,6,6)
        x = F.max_pool2d(F.leaky_relu(self.conv2(x)), (2,2))
        
        #Pixelwise
        x = F.leaky_relu(self.pxl3(x))
        x = F.leaky_relu(self.pxl4(x))
        
        #(16,6,6) -> (32,4,4)
        x = F.leaky_relu(self.conv3(x))
        
        #Pixelwise
        x = F.leaky_relu(self.pxl5(x))
        x = F.leaky_relu(self.pxl6(x))
        
        #(32,4,4) -> (64,2,2)
        x = F.leaky_relu(self.conv4(x))
        
        #Pixelwise
        x = F.leaky_relu(self.pxl7(x))
        x = F.leaky_relu(self.pxl8(x))
