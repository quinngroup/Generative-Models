...
if __name__ == "__main__":
    ...
    parser.add_argument('--es', action='store_true', default= False,
                        help='run earlystopping')
    parser.add_argument('--patience', type = int, default=4, metavar='pt',
                        help='Patience parameter for early stopping: Unused if --es false')
    parser.add_argument('--mindelta', action='store_true', default=0)
    ...
    if(args.load == ''):
        ...
        if(args.es):
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                if (patience == 0) :
                    benchmarkLoss = averageLoss
                if (averageLoss >= benchmarkLoss):
                    pt += 1
                elif (averageLoss < benchmarkLoss):
                    patience = 0
